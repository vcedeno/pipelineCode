    ...
    for phase in phasesids:  	
    	...
    	if str(nvalue)==str(n) and str(dvalue)==str(d):
    		...
    		for player in players:
    			...
    			playersactions.append([player,letterstring,eventssplayer])
...
def main(experimentfile,phasedescfile,phasefile,actionfile,playerfile,featureid,windowsize,nd,numseconds):
    ...
    featurejson=open(os.getcwd()+'/data-analytics-pipeline/json-schema/h13/input/datasets/h13.json', 'w')
    featurejson2=open(os.getcwd()+'/property-inference-pipeline/json-schema/inputTransformation/datasets/h13.json', 'w')
    ...
    featuresjsondata=[{"features":[{"n":int(row[0]),"d":int(row[1]),"windowsize":int(row[2]),"numseconds":int(row[3]),"phases":[{"phaseid":str(row2[0]),"players":[{"playerid":str(row3[0]),"initialLetters":str(row3[1]),"timeline":[{"requestsSent":str(row4[0]),"repliesReceived":str(row4[1]),"requestsReceived":str(row4[2]),"repliesSent":str(row4[3]),"words":str(row4[4])} for row4 in row3[2]]}for row3 in row2[1]]}for row2 in row[4]]}]} for row in allfeaturesjsondata]
    json.dump(featuresjsondata,featurejson,indent=2)
    json.dump(featuresjsondata,featurejson2,indent=2)
    print (" -- h13 Transformation --")
